...
public class MessagerMg extends TextComponentSerializer {
     ...
     public static void messageAgain(Player player){
          TextComponent component1 = new TextComponent(UT.getString("StunMessage.MessageAgain"));
         ...
         component1.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                 new ComponentBuilder(UT.getString("StunMessage.MessageAgain1")).color(ChatColor.GOLD).italic(true).create()));
         player.spigot().sendMessage(component1);
     }
    public static void messageDelete(Player player){
        TextComponent component1 = new TextComponent(UT.getString("StunMessage.RestartMessage"));
        component1.setColor(ChatColor.GOLD);
        component1.setBold(true);
        component1.setClickEvent(new ClickEvent(ClickEvent.Action.RUN_COMMAND, "/roul remove"));
        component1.setHoverEvent(new HoverEvent(HoverEvent.Action.SHOW_TEXT,
                new ComponentBuilder(UT.getString("StunMessage.RestartMessage1")).color(ChatColor.GOLD).italic(true).create()));
        player.spigot().sendMessage(component1);
    }
}
